...
def test_figure_notebooks_are_syntactically_valid() -> None:
    """Every generated notebook's code parses.

    The notebooks are generated by templating Python source through two levels of
    string escaping, which is exactly the kind of pipeline that produces a stray
    double comma or a collapsed ``\\n`` inside an f-string. Both happened. Parsing
    is instant and needs no kernel, so there is no reason to discover it by
    executing a notebook minutes later.
    """
    ...
    notebooks = sorted(
        (
            pathlib.Path(__file__).resolve().parents[2] / "examples" / "jaccpot_paper"
        ).glob("fig_*.ipynb")
    )
    assert notebooks, "no figure notebooks found; run build_notebooks.py"
        ...
        for index, cell in enumerate(payload["cells"]):
            ...
            try:
                ...
                raise AssertionError(
                    f"{path.name} cell {index} line {exc.lineno}: {exc.msg}"
                ) from exc


def test_figure_notebooks_never_recompute() -> None:
    """A figure notebook must not import the solver or JAX.

    This is the layering rule made mechanical rather than aspirational: figures
    read ``results/*.json`` and draw. A notebook that can import jaccpot can
    quietly start recomputing a number, and then the figure and the artifact it
    claims to plot are free to disagree.
    """
    ...
    forbidden = re.compile(r"^\s*(?:import|from)\s+(jax|jaccpot|yggdrax)\b", re.M)
    notebooks = sorted(
        (
            pathlib.Path(__file__).resolve().parents[2] / "examples" / "jaccpot_paper"
        ).glob("fig_*.ipynb")
    ...
    for path in notebooks:
        ...
        for cell in payload["cells"]:
            if cell.get("cell_type") != "code":
                continue
            hit = forbidden.search("".join(cell["source"]))
            if hit:
                offenders[path.name] = hit.group(0).strip()
    assert not offenders, (
        "figure notebooks import compute libraries, so they could recompute "
        f"instead of plotting: {offenders}"
    )


def test_notebook_string_literals_survive_the_escaping() -> None:
    r"""No notebook string literal evaluates to contain a control character.

    The notebooks are generated by templating Python source inside a Python
    triple-quoted string, so every backslash is consumed twice. A LaTeX label
    written ``$\theta$`` with two backslashes reaches the notebook as ``\theta``,
    which Python then evaluates to TAB + "heta" -- and it renders in the figure as
    "sweep     heta". It is silent: the notebook parses, executes, and produces a
    plot with a wrong label.

    Scanning the raw cell text cannot see this, because the corruption only exists
    after Python evaluates the literal. So walk the AST and check the *values*.
    This class of bug has appeared three times on this branch (a collapsed ``\n``
    in figure 01, another in figure 13, and this tab in figure 03).
    """
        ...
        for cell in payload["cells"]:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Constant) and isinstance(node.value, str):
                    found = [n for c, n in control.items() if c in node.value]
                    if found:
                        offenders.append(
                            f"{path.name}:{node.lineno} {found} in {node.value[:60]!r}"
                        )
    assert not offenders, (
        "notebook string literals contain control characters, i.e. a backslash "
        "escape was eaten by the generator's own quoting (use four backslashes for "
        "a LaTeX macro): " + "; ".join(offenders)
    )
