...
def build(donate: bool):
    ...
    sm = shard_map(
        body, mesh=mesh, in_specs=(P("gpus"),) * 5, out_specs=P("gpus"), check_vma=False
    )
...
def main():
    ...
    ap.add_argument(
        "--churn",
        action="store_true",
        help="hold varying-size junk alive across each call",
    )
    ...
    for i in range(a.iters):
        ...
        if a.churn:
            junk = [
                jnp.ones((int(rng.integers(1 << 12, 1 << 24)),), jnp.float64)
                for _ in range(3)
            ]
    ...
    print("  " + "".join(pattern))
    print(
        f"  -> {'CORRUPT' if bad else 'CLEAN'} ({bad}/{a.iters} calls returned the fill value or garbage)"
    )
