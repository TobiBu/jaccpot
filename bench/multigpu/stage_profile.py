...
the then-separate ``theta_cross``, toggling the fused-M2L flag, and sweeping the
near-field cap and
...
INERT AS OF 2026-09-02: `jaccpot/distributed/fmm.py` currently emits NO `jc_*`
scopes. The paper branch added eight of them, but `main` rewrote that file and the
rebase took `main`'s version -- nothing committed depended on the scopes, and this
tool already expected three stages (`jc_coarse_m2m`, `jc_l2p`, `jc_near_p2p_self`)
that were never emitted anyway. Re-instrument the current call sites before
...
(the then-separate ``theta_cross``, the fused-M2L flag, the near-field cap, the
traversal queue, and
