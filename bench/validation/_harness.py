...
def make_distribution(name: str, n: int, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    if name == "disc":
        ...
        #
        # Ported here from bench/validation/mac_error_distribution.py when the paper
        # branch's pure-move refactor extracted make_distribution into this module.
        # main added the case while the branch was moving the function; keeping only
        # one side would have silently dropped it.
