...
from jaccpot.pallas.nearfield_fused_leaf import nearfield_leafpair_pallas, nearfield_leafpair_jax
...
pos = np.zeros((L, W, 3)); mass = np.zeros((L, W))
...
def call(dt, **kw):
    a = (jnp.asarray(pos, dt), jnp.asarray(mass, dt), jnp.asarray(mask),
...
ref = np.asarray(nearfield_leafpair_jax(
    jnp.asarray(pos, jnp.float64), jnp.asarray(mass, jnp.float64), jnp.asarray(mask),
    jnp.asarray(sids), jnp.asarray(svalid),
    softening_sq=jnp.asarray(1e-6, jnp.float64), G=jnp.asarray(1.0, jnp.float64)))
...
acc = slice(0, 3)
def relerr(g):
    return float(np.linalg.norm(g[0, :, acc] - ref[0, :, acc]) / np.linalg.norm(ref[0, :, acc]))
e_in, e_wide = relerr(default), relerr(widened)
tot = float(np.abs(ref[0, :, acc]).sum()); net = float(np.linalg.norm(ref[0, :, acc]))
print(f"   cancellation factor sum|a|/|sum a| ~ {tot/max(net,1e-300):.1f}  ({S} source leaves x {W} lanes)")
print(f"2. rel err  input {e_in:.3e}   wide {e_wide:.3e}   improvement {e_in/max(e_wide,1e-300):.1f}x")
assert e_wide < e_in / 3.0, f"widened accumulator did not help: {e_in:.3e} -> {e_wide:.3e}"
